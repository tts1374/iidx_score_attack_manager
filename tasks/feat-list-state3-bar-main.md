...
- レビュー対応として、一覧3分割バーの色コードを詳細画面の3分割バー色と一致させる。
- レビュー対応として、一覧の残り日数バッジ文言を詳細画面の表示（`残り{{count}}日` / `今日まで`）と一致させる。
...
- `packages/web-app/src/pages/HomePage.tsx`
  ...
  - 残り日数バッジは `resolveTournamentCardStatus` の `label` を直接表示して詳細と統一する。
