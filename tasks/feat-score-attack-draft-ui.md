...
- レビュー反映:
  - ステップ直下の状態表示を「背景/枠なし・stickyなし」の1行テキストへ調整。
  - 譜面ステップの未入力エラーをカード内表示のみに統一（グローバルエラー表示を廃止）。
  - 難易度ボタンを維持しつつ `B/N/H/A/L + 数値` の短縮表示へ変更し、縦幅を圧縮。
  - 確認画面の曲名を2行省略表示にし、長文崩れを防止。
  - 曲名/ハッシュタグをコピー可能にする導線を追加。
  - デバッグ情報露出をデバッグモードON時のみに統一。
  - ハッシュタグコピー導線を削除して通常テキスト表示のみに戻す。
  - 確認画面の大会ID（def_hash）常時表示を復活し、短縮表示 + コピーアイコンのみ提供する。
  - 確認画面の大会ID短縮表示を先頭30文字に拡張する。
  - 確認画面の譜面カードコピー導線を削除する。
  - 補助情報（入力完了/未入力）行の余白・line-height・font-sizeを指定値に合わせる。
  - 次へ/戻る/nav操作時に作成画面の先頭へスクロールさせる。
  - `.createTournamentPage` の `gap` 起因の32px空白を解消し、`createWizardSteps` と `section` の上下余白を `8px` に統一する。
  - 期間入力の「来月（1日〜月末）」を主ボタンから補助ショートカットリンクの見た目へ変更し、期間入力ブロック内に配置する。
  - 期間入力欄（DatePicker）の背景色を他入力欄と同じ白背景に統一する。
...
- 作成画面:
  - 進捗ボックスが消え、ステップ直下1行状態表示になる（背景/枠なし、stickyなし）。
  - 譜面の未入力エラーはカード内のみ表示され、グローバルエラーは出ない。
  - 難易度ボタンが `B 2 / N 6 / H 10 / A 12 / L 12` 表記になる。
  - ラベルが通常テキスト表示になる。
  - 確認画面譜面表示が `SP ANOTHER 10` 形式（Lvなし、スラッシュなし）になる。
  - 曲名が2行省略され、長文でレイアウト崩れが起きない。
  - ハッシュタグのコピーUIが存在しない。
  - 確認画面に大会ID（短縮表示 + コピーアイコン）が表示される。
  - 大会ID短縮表示が先頭30文字になっている。
  - 譜面カードにコピーUIが存在しない。
  - 次へ/戻る/navで作成画面の先頭が表示される。
  - `createWizardSteps` と `section` 間の縦余白が `8px` ベースで表示される。
  - 期間入力の「来月（1日〜月末）」が補助リンクの見た目で表示され、主CTAより目立たない。
  - 画面幅が狭い場合、期間ショートカットは期間入力直下に回り、タップしやすさを維持する。
  - 期間入力欄の背景色が他入力欄と統一される。
